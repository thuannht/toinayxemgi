import assert from 'node:assert/strict';
const url = process.env.COUNTER_TEST_URL || 'http://127.0.0.1:8788/spins';
const origin = 'http://127.0.0.1:4173';
const read = async () => (await (await fetch(url)).json()).count;
const post = (id, weight) => fetch(url, { method:'POST', headers:{'Content-Type':'text/plain;charset=UTF-8',Origin:origin}, body:JSON.stringify(weight ? {id,weight} : {id}) });
const initial = await fetch(url);
assert.match(initial.headers.get('Cache-Control') || '', /max-age=5/);
const before = await read();
const ids = Array.from({length:24}, () => crypto.randomUUID());
const responses = await Promise.all(ids.flatMap(id => [post(id),post(id),post(id.toUpperCase())]));
assert.ok(responses.every(r => r.ok));
assert.equal((await post(crypto.randomUUID(), 8)).status, 200);
assert.equal((await post(crypto.randomUUID(), 7)).status, 400);
await new Promise(resolve => setTimeout(resolve, 5_100));
assert.equal(await read(), before + responses.length + 8, 'Legacy and sampled requests must increment atomically with their allowed weights');
assert.equal((await post('not-a-uuid')).status,400);
assert.equal((await fetch(url,{method:'POST',headers:{'Content-Type':'application/json'},body:'x'.repeat(300)})).status,413);
assert.equal((await fetch(url,{headers:{Origin:'https://other.example'}})).status,403);
const preflight=await fetch(url,{method:'OPTIONS',headers:{Origin:origin}});
assert.equal(preflight.status,204);assert.equal(preflight.headers.get('Access-Control-Allow-Origin'),origin);
assert.equal(await read(),before+responses.length+8,'Invalid/read-only requests must not increment');
console.log('PASS: edge cache; legacy + sampled atomic writes; invalid weight, malformed, oversized, CORS and read-only cases passed.');
